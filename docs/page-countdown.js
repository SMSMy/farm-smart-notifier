...
class PageCountdownTimer {
    ...
    createContainer() {
        ...
        if (cardElement) {
            ...
            if (cardTitle) {
                cardTitle.parentNode.insertBefore(this.container, cardTitle.nextSibling);
            } else {
    ...
    findRelevantNotification() {
        if (!this.notificationData.notifications) return null;
        ...
        const pageTypeMapping = {
            ...
            'pipe_waterer': ['pipe_waterer_change_water', 'pipe_waterer_rinse', 'pipe_waterer_sanitize', 'pipe_waterer_deep_clean'],
            'henna': ['fertilizer'],
            'fig': ['fertilizer'],
            'banana': ['fertilizer'],
            'mango': ['fertilizer'],
    ...
    renderCountdown(notification) {
        ...
        const title = isArabic ? 'الإشعار القادم لهذه المهمة' : 'এই কাজের পরবর্তী বিজ্ঞপ্তি';
